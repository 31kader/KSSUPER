...
import { cn, generateUniqueId, logAction, sanitizeProductForSupabase } from '../lib/utils';
...
export function ProductFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const catId = submitPayload.category_id;
      if (catId) {
        const { data: existingCat, error: catCheckErr } = await supabase
          .from('categories')
          .select('id')
          .eq('id', catId)
          .single();
        
        if (catCheckErr || !existingCat) {
          console.warn("Category not found in database, defaulting to 'uncategorized':", catId);
      ...
      if (brandId) {
        const { data: existingBrand, error: brandCheckErr } = await supabase
          .from('brands')
          .select('id')
          .eq('id', brandId)
          .single();
          
        if (brandCheckErr || !existingBrand) {
      ...
      if (editingProduct && editingProduct.id) {
        ...
        // Non-blocking Supabase update using robust submitPayload
        supabase.from('products').update(submitPayload).eq('id', editingProduct.id)
          .then(({ error }) => {
        ...
        submitPayload.id = newProductId;
        submitPayload.created_at = createdProduct.createdAt;

        // Non-blocking Supabase insert using robust submitPayload
        supabase.from('products').insert(submitPayload)
          .then(({ error }) => {
                ...
                onClick={() => {
                  ...
                  setFormData(prev => {
                    ...
                    if (nextVal) {
                      ...
                      const totalBatchStock = currentBatches.reduce((sum, b) => sum + b.stock, 0);
                      ...
                      if (prev.batches && prev.batches.length > 0) {
                        ...
                        updated.stock = prev.batches.reduce((sum, b) => sum + b.stock, 0).toString();
                      }
                    }
                    return updated;
                  });
                          ...
                          <tr key={b.id || index} className="hover:bg-white/[0.01]">
                            ...
                            <td className="py-3 px-4 text-right">
                              <button
                                ...
                                onClick={() => {
                                  setFormData(prev => {
                                    const filtered = prev.batches.filter((_, idx) => idx !== index);
